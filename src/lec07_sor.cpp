...
int main() {
    ...
    // Load input PCD file
    if (pcl::io::loadPCDFile<pcl::PointXYZ>("/path/gachon.pcd", *cloud) == -1) {
    ...
    pcl::io::savePCDFile("/path/to/original.pcd", *cloud);
    pcl::io::savePCDFile("/path/to/inliers.pcd", *cloud_filtered);
    sor.setNegative(true);
    sor.filter(*cloud_filtered);
    pcl::io::savePCDFile("/path/to/outliers.pcd", *cloud_filtered);
    ...
    cv::waitKey(0);
    cv::imwrite("/path/to/2d_projection.png", projection_image);
